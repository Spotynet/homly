...
from .models import (
    ...
    Notification, DocumentCategory, Document,
)
...
class DocumentCategoryViewSet(viewsets.ModelViewSet):
    """CRUD /api/tenants/{tenant_id}/document-categories/"""
    ...
    def _is_admin(self):
        ...
        tu = TenantUser.objects.filter(
            tenant_id=self.kwargs.get('tenant_id'), user=self.request.user
        ).first()
        return tu and tu.role == 'admin'

    def get_queryset(self):
        return DocumentCategory.objects.filter(tenant_id=self.kwargs['tenant_id'])
    ...
    def create(self, request, *args, **kwargs):
        if not self._is_admin():
            return Response({'detail': 'Solo el administrador puede crear categorías.'}, status=status.HTTP_403_FORBIDDEN)
        return super().create(request, *args, **kwargs)

    def update(self, request, *args, **kwargs):
        if not self._is_admin():
            return Response({'detail': 'Solo el administrador puede editar categorías.'}, status=status.HTTP_403_FORBIDDEN)
        return super().update(request, *args, **kwargs)

    def destroy(self, request, *args, **kwargs):
        if not self._is_admin():
            return Response({'detail': 'Solo el administrador puede eliminar categorías.'}, status=status.HTTP_403_FORBIDDEN)
...
class DocumentViewSet(viewsets.ModelViewSet):
    """CRUD /api/tenants/{tenant_id}/documents/"""
    permission_classes = [IsTenantMember]
    ...
    def _can(self, doc, action):
        """Check 'read' | 'write' | 'delete' permission for requesting user on a document."""
        role = self._user_role()
        if role == 'admin' or self.request.user.is_super_admin:
            return True
        if not role:
            return False
        perms = doc.permissions or {}
        return bool(perms.get(role, {}).get(action, False))

    # ── Serializer selection ──────────────────────────────────

    def get_serializer_class(self):
        # Use lightweight serializer for list; full for retrieve/create/update
        if self.action == 'list':
    ...
    def get_queryset(self):
        tenant_id = self.kwargs['tenant_id']
        role = self._user_role()
        qs = Document.objects.filter(tenant_id=tenant_id).select_related('category')

        # Non-admins only see published docs
        if role != 'admin' and not self.request.user.is_super_admin:
        ...
        # Permission-based filtering for non-admins: keep only readable docs
        if role and role != 'admin' and not self.request.user.is_super_admin:
            readable_ids = [
                doc.id for doc in qs
                if bool((doc.permissions or {}).get(role, {}).get('read', False))
            ]
            qs = Document.objects.filter(id__in=readable_ids).select_related('category')
    ...
    def perform_create(self, serializer):
        tenant_id = self.kwargs['tenant_id']
        created_by = self.request.user.name or self.request.user.email or ''
        perms = serializer.validated_data.get('permissions') or {}
        if not perms:
            perms = Document.default_permissions()
        serializer.save(tenant_id=tenant_id, created_by_name=created_by, permissions=perms)

    def create(self, request, *args, **kwargs):
        role = self._user_role()
        if role not in ('admin', 'tesorero') and not request.user.is_super_admin:
            return Response(
    ...
    def toggle_published(self, request, tenant_id=None, pk=None):
        """Toggle published/draft state. Admin only."""
        role = self._user_role()
        if role != 'admin' and not request.user.is_super_admin:
            return Response({'detail': 'Solo el administrador puede publicar/despublicar documentos.'}, status=status.HTTP_403_FORBIDDEN)
        doc = self.get_object()
        doc.published = not doc.published
        doc.save(update_fields=['published'])
        return Response({'published': doc.published})

    @action(detail=True, methods=['post'], url_path='duplicate')
    def duplicate(self, request, tenant_id=None, pk=None):
        """Duplicate a document (useful for templates)."""
        role = self._user_role()
        if role not in ('admin', 'tesorero') and not request.user.is_super_admin:
            return Response({'detail': 'No tiene permisos para duplicar documentos.'}, status=status.HTTP_403_FORBIDDEN)
        doc = self.get_object()
        new_title = request.data.get('title', f'Copia de {doc.title}')
        new_doc = Document.objects.create(
            ...
            published=False,
            created_by_name=request.user.name or request.user.email or '',
        )
