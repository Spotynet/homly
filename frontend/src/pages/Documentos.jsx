...
function DocumentEditorModal({ doc, categories, onClose, onSave }) {
  ...
  // Set initial HTML content once on mount (avoids dangerouslySetInnerHTML + contentEditable conflict)
  useEffect(() => {
  ...
  const insertTable = useCallback(() => {
    ...
    const html = `<table border="1" style="border-collapse:collapse;width:100%;margin:12px 0;">${rows}</table><p><br></p>`;
  ...
  const handlePrintPreview = () => {
    ...
    win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8">
      ...
      <style>
        body{font-family:Georgia,serif;padding:40px;max-width:800px;margin:0 auto;line-height:1.7;color:#1a1a1a;}
    ...
    <>
      <div className="modal-bg open" onClick={onClose}>
        <div
          ...
          style={{ maxWidth:920, width:'97vw', maxHeight:'95vh', display:'flex', flexDirection:'column' }}>

          {/* Head */}
          <div className="modal-head">
            <h3>
              <FilePlus2 size={15} style={{ display:'inline', verticalAlign:-2, marginRight:6 }} />
          ...
          <div style={{ display:'flex', gap:10, padding:'10px 16px',
            borderBottom:'1px solid var(--sand-200)', flexWrap:'wrap', alignItems:'center' }}>
            <input className="field-input" value={form.title} style={{ flex:2, minWidth:180 }}
              onChange={e => setForm(p => ({ ...p, title: e.target.value }))}
              placeholder="Título del documento *" />
            <input className="field-input" value={form.description} style={{ flex:2, minWidth:160 }}
              onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
              placeholder="Descripción (opcional)" />
            <select className="field-input" value={form.category} style={{ flex:1, minWidth:130 }}
              onChange={e => setForm(p => ({ ...p, category: e.target.value }))}>
              <option value="">Sin categoría</option>
              {categories.map(c => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}
            </select>
          ...
          <div style={{
            ...
            <select
              ...
              style={{ height:28, fontSize:12, border:'1px solid var(--sand-200)', borderRadius:4,
                background:'white', cursor:'pointer', marginRight:2 }}>
            ...
            <ToolBtn title="Negrita (Ctrl+B)"   onExec={() => exec('bold')}><strong>N</strong></ToolBtn>
            <ToolBtn title="Cursiva (Ctrl+I)"   onExec={() => exec('italic')}><em>K</em></ToolBtn>
            <ToolBtn title="Subrayado (Ctrl+U)" onExec={() => exec('underline')}><u>S</u></ToolBtn>
            <ToolBtn title="Tachado"            onExec={() => exec('strikeThrough')}><s>T</s></ToolBtn>

            <ToolSep />

            <ToolBtn title="Alinear izquierda" onExec={() => exec('justifyLeft')}>
              <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.5">
                <line x1="2" y1="3" x2="12" y2="3"/><line x1="2" y1="7" x2="9" y2="7"/><line x1="2" y1="11" x2="12" y2="11"/>
              </svg>
            </ToolBtn>
            <ToolBtn title="Centrar" onExec={() => exec('justifyCenter')}>
              <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.5">
                <line x1="2" y1="3" x2="12" y2="3"/><line x1="4" y1="7" x2="10" y2="7"/><line x1="2" y1="11" x2="12" y2="11"/>
              </svg>
            </ToolBtn>
            <ToolBtn title="Alinear derecha" onExec={() => exec('justifyRight')}>
              <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.5">
                <line x1="2" y1="3" x2="12" y2="3"/><line x1="5" y1="7" x2="12" y2="7"/><line x1="2" y1="11" x2="12" y2="11"/>
              </svg>
            </ToolBtn>

            <ToolSep />

            <ToolBtn title="Lista con viñetas"  onExec={() => exec('insertUnorderedList')}>
              <svg width="14" height="14" fill="currentColor" stroke="none">
                <circle cx="3" cy="4" r="1.3"/><circle cx="3" cy="10" r="1.3"/>
                <rect x="6" y="3" width="6" height="1.5" rx="0.5"/>
                <rect x="6" y="9" width="6" height="1.5" rx="0.5"/>
              </svg>
            </ToolBtn>
            <ToolBtn title="Lista numerada"     onExec={() => exec('insertOrderedList')}>
              <svg width="14" height="14" fill="currentColor" stroke="none">
                <text x="1" y="6" fontSize="6" fontWeight="bold">1.</text>
                <text x="1" y="12" fontSize="6" fontWeight="bold">2.</text>
                <rect x="7" y="3" width="6" height="1.5" rx="0.5"/>
                <rect x="7" y="9" width="6" height="1.5" rx="0.5"/>
              </svg>
            </ToolBtn>
            <ToolBtn title="Aumentar sangría"   onExec={() => exec('indent')}>→</ToolBtn>
            <ToolBtn title="Reducir sangría"    onExec={() => exec('outdent')}>←</ToolBtn>

            <ToolSep />

            {/* Text color */}
            <label title="Color de texto" style={{ display:'flex', alignItems:'center', gap:2, cursor:'pointer' }}>
              <span style={{ fontSize:13, fontWeight:800, color:'var(--ink-600)' }}>A</span>
              <input type="color" defaultValue="#000000"
                onChange={e => exec('foreColor', e.target.value)}
                style={{ width:18, height:18, border:'none', cursor:'pointer', padding:0, background:'none' }} />
            </label>

            {/* Highlight */}
            <label title="Color de fondo" style={{ display:'flex', alignItems:'center', gap:1, cursor:'pointer' }}>
              <span style={{ fontSize:10, color:'var(--ink-400)' }}>bg</span>
            ...
            <ToolBtn title="Insertar tabla"  onExec={insertTable}>⊞ Tabla</ToolBtn>
            <ToolBtn title="Insertar imagen" onExec={insertImage}>🖼 Imagen</ToolBtn>

            <ToolSep />

            <ToolBtn title="Deshacer (Ctrl+Z)" onExec={() => exec('undo')}>↩</ToolBtn>
            <ToolBtn title="Rehacer (Ctrl+Y)"  onExec={() => exec('redo')}>↪</ToolBtn>
          ...
          {/* Editor area — NO dangerouslySetInnerHTML (content set via useEffect) */}
          <div
            ref={editorRef}
            contentEditable
            ...
            style={{
              ...
              outline: 'none', minHeight: 250, background: 'white',
            }}
          />
          ...
          <div className="modal-foot">
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <input type="checkbox" id="editor_tmpl" checked={form.is_template}
                onChange={e => setForm(p => ({ ...p, is_template: e.target.checked }))}
                style={{ width:15, height:15, accentColor:'var(--teal-500)', cursor:'pointer' }} />
              <label htmlFor="editor_tmpl" style={{ fontSize:12, cursor:'pointer', color:'var(--ink-600)' }}>
